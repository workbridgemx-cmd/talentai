export async function onRequestPost(context) {
  ...
  try {
    const { text, filename, apiKey, config } = await context.request.json();

    if (!apiKey || apiKey.trim().length < 20) {
      return new Response(JSON.stringify({ ok: false, error: 'API Key inválida o vacía' }), { headers: corsHeaders });
    }
    if (!text || text.trim().length < 80) {
      return new Response(JSON.stringify({ ok: false, error: 'Texto del CV muy corto o vacío' }), { headers: corsHeaders });
    }
...
Score 0-100: experiencia ${w.exp}%, técnico ${w.tec}%, idiomas ${w.idi}%, estabilidad ${w.est}%, soft skills ${w.soft}%, educación ${w.edu}%.`;
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ ok: false, error: err.error?.message || 'Error API ' + response.status }), { headers: corsHeaders });
    }
    ...
    return new Response(JSON.stringify({ ok: true, data: parsed }), { headers: corsHeaders });

  } catch (err) {
    return new Response(JSON.stringify({ ok: false, error: err.message }), { headers: corsHeaders, status: 500 });
  }
}
